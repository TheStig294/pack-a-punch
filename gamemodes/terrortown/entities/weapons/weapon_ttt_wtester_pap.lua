...
function SWEP:GatherRagdollSample(ent)
    local sample = ent.killer_sample or {
        t = 0,
        killer = nil
    }
...
function SWEP:GatherObjectSample(ent)
    if ent:GetClass() == "ttt_c4" and ent:GetArmed() then
        ...
        local collected = self:AddItemSample(ent)

        if collected == -1 then
            self:Report("dna_limit")
        else
            self:Report("dna_object", {
                num = collected
            })
        end
    end
...
local function GetTester(ply)
    if IsValid(ply) then
        ...
        if IsValid(tester) and tester:GetClass() == "weapon_ttt_wtester" then return tester end
    end
